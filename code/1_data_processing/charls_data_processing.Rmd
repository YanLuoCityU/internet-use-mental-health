...
wide_harm_charls_s <- harm_charls %>%
  dplyr::select(
    c(ID) |
      (starts_with("s") & (ends_with("livpar"))) # Number of living parents
     )

index_reorder <- str_detect(names(wide_harm_charls_s), "[h-s][0-9]+(.*)")
names_reorder <- names(wide_harm_charls_s)[index_reorder] %>% 
  str_match("[h-s]([0-9]+)(.*)") %>% 
  as_tibble() %>% 
...
long_charls_s <- wide_harm_charls_s %>% 
...
wide_harm_charls <- harm_charls %>%
  dplyr::select(
    c(ID, householdID, hhid, pn, pnc, ID_w1, householdID_w1, communityID, ragender, raeducl, radyear, radmonth) | # id, gender, education
      ...
      (starts_with("h") & (ends_with("hhres") | ends_with("child") | ends_with("pcnt") | ends_with("kcnt"))) | # Household size, number of children, contact with other people
...
long_charls <- wide_harm_charls %>% 
  dplyr::select(!contains(names_reorder), contains(names_reorder)) %>%
    pivot_longer(
    col = !c(ID, householdID, hhid, pn, pnc, ID_w1, householdID_w1, communityID, ragender, raeducl, radyear, radmonth),
    names_to = ".value",
  ...
  mutate(followup = row_number()) %>%
  ungroup() %>%
  left_join(., long_charls_s, by = c("ID", "followup")) %>%
  dplyr::select(!c(followup)) %>%
  remove_all_labels() %>%
  zap_formats()

rm(harm_charls, wide_harm_charls, wide_harm_charls_s, index_reorder, names_reorder)
...
tidy_charls <- long_merge %>%
  ...
  mutate( 
    ...
    ## Outcomes
    # depressive symptom
    depresl = depresl -1, flonel = flonel - 1, effortl = effortl - 1, 
    sleeprl = sleeprl - 1, whappyl = reverse_code(whappyl - 1), 
    botherl = botherl - 1, mindtsl = mindtsl - 1, 
    fhopel = reverse_code(fhopel - 1), fearll = fearll - 1, goingl = goingl - 1,
    # self-rated health
...
id_ot_bl <- intersect(id_cesd_bl, id_satlife_bl) # 13043
length(unique(id_ot_bl))
id_ot_bl <- intersect(id_ot_bl, id_shlt_bl) # 13036
length(unique(id_ot_bl))
...
id_ot <- intersect(id_cesd, id_satlife) # 11127
length(unique(id_ot))
id_ot <- intersect(id_ot, id_shlt) # 11124
length(unique(id_ot))
...
length(unique(filter_cov_iyes$id)) # 3002
...
final <- filter_cov_iyes %>%
  ...
  mutate(
    ...
    depression = ifelse(cesd >= 10, 1, 0), cwave
  ) %>%
  dplyr::select(
    id, country, iwy, iwm, wave,
    cesd, shlt, satlife,
...
attr_ipw <- tidy_charls %>% 
  select(
    id, wave,
    depresl, flonel, effortl, sleeprl, whappyl, botherl, mindtsl, fhopel, fearll, goingl,
    satlife, shlt
  ) %>%
  left_join(., bl, by = "id")

# Construct attrition observations
attr_obs <- tidy_charls %>% 
  group_by(id) %>%
  slice(n()) %>%
  ungroup() %>%
  filter(wave != 2018) %>%
...
final_iyes_charls <- final_iyes_charls %>%
...
length(unique(final_iyes_charls$id)) # 3002

# Keep participants without depression at baseline
final_iyes_inc <- final_iyes_charls %>%
  group_by(id) %>%
  ...
  ungroup()
length(unique(final_iyes_inc$id)) # 1592
...
# Final dataset
final_iyes_inc_charls <- bind_rows(final_iyes_inc_dep, final_iyes_inc_nodep) %>%
...
tbl %>%
  as_flex_table() %>%
  flextable::save_as_docx(path = str_c(getwd(), "/results/Table/Supplementary Table 5_Differences in baseline characteristics between analytic and whole samples_CHARLS.docx"))
...
id_ot_2w <- intersect(id_cesd_2w, id_satlife_2w) # 11563
length(unique(id_ot_2w)) 
id_ot_2w <- intersect(id_ot_2w, id_shlt_2w) # 11555
length(unique(id_ot_2w)) 
...
id_ot <- intersect(id_cesd, id_satlife) # 9362
length(unique(id_ot)) 
id_ot <- intersect(id_ot, id_shlt) # 9356
length(unique(id_ot))
...
filter_cov <- filter_nomental %>%
  ...
  ungroup()
length(unique(filter_cov$id)) # 2443
...
icum_gformula_charls <- filter_cov %>%
  group_by(id) %>%
...
length(unique(filter_iyes_bl$id)) # 16903
...
filter_iyes <- filter_2obs %>%
  filter(id %in% id_iyes)
length(unique(filter_iyes$id)) # 11833
...
filter_nomental <- filter_iyes %>%
  filter(!id %in% id_mental) 
length(unique(filter_nomental$id)) # 11536
...
filter_cov <- filter_nomental %>%
  ...
  ungroup()
length(unique(filter_cov$id)) # 3157
...
# Weekly contact with other people
contact_iyes_charls <- final_iyes_charls %>%
  filter(contact == "yes") %>%
  select(!c(cesd_stand, shlt_stand, satlife_stand)) %>%
  mutate(
    cesd_stand = scale(cesd), shlt_stand = scale(shlt), satlife_stand = scale(satlife)
...
# No weekly contact
nocontact_iyes_charls <- final_iyes_charls %>%
  filter(contact == "no") %>%
  select(!c(cesd_stand, shlt_stand, satlife_stand)) %>%
  mutate(
    cesd_stand = scale(cesd), shlt_stand = scale(shlt), satlife_stand = scale(satlife)
...
mi_iyes_inc_charls <- list()
for(i in 1:10){
  mi_data <- mice::complete(mi_iyes, i) %>%
    mutate(depression = ifelse(cesd >= 10, 1, 0)) %>%
    group_by(id) %>%
    filter(first(depression == 0)) %>%
    mutate(
      has_depression = any(depression == 1, na.rm = T)
  ...
  mi_iyes_inc_charls[[i]] <- bind_rows(mi_data_dep, mi_data_nodep) %>%
...
predictorMatrix <- matrix(0, nrow = dim(icum_gformula_charls)[2], ncol = dim(icum_gformula_charls)[2])
colnames(predictorMatrix) <- colnames(icum_gformula_charls)
rownames(predictorMatrix) <- colnames(icum_gformula_charls)
row_pre <- which(rownames(predictorMatrix) %in% colnames(icum_gformula_charls)[-c(1:3)])
col_pre <- which(colnames(predictorMatrix) %in% colnames(icum_gformula_charls)[-c(1:3)])
predictorMatrix[row_pre, col_pre] <- 1
for(i in 4:dim(icum_gformula_charls)[2]){ predictorMatrix[i,i]<-0 }

cores <- detectCores()
tic()
# mi_icum_gformula <- icum_gformula_charls %>%
#   futuremice(data = ., m = 5, maxit = 5, pred = quickpred(icum_gformula_charls), parallelseed = 12345, n.core = cores - 1)
mi_icum_gformula <- icum_gformula_charls %>%
  futuremice(data = ., m = 10, maxit = 5, predictorMatrix = predictorMatrix, parallelseed = 12345, n.core = cores - 1, method = "cart")
toc()

mi_icum_gformula_charls <- list()
for(i in 1:10){
  ...
  mi_icum_gformula_charls[[i]] <- mi_data
...
final_mi_bidirec <- final_bidirec_charls %>%
...
save(
  final_iyes_charls, final_ifreq_charls, final_icum_charls, 
  final_iyes_inc_charls, icum_gformula_charls, final_bidirec_charls, # Complete cases
  middle_iyes_charls, old_iyes_charls, male_iyes_charls, female_iyes_charls,
  married_iyes_charls, nonmarried_iyes_charls, contact_iyes_charls, nocontact_iyes_charls,
  ...
  mi_iyes_charls, mi_ifreq_charls, mi_icum_charls, mi_iyes_inc_charls, 
  mi_icum_gformula_charls, mi_bidirec_charls, # Multiple imputation analyses
