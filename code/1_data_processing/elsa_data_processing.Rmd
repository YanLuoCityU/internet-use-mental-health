...
wide_harm_elsa <- harm_elsa %>%
  as_tibble() %>%
  dplyr::select(
    c(idauniq, idauniqc, pn, pnc, ragender, raeducl) | # id, gender, education
...
long_elsa <- wide_harm_elsa %>% 
  dplyr::select(!contains(names_reorder), contains(names_reorder)) %>%
    pivot_longer(
    col = !c(idauniq, idauniqc, pn, pnc, ragender, raeducl),
    names_to = ".value",
...
id_ot_bl <- intersect(id_cesd_bl, id_shlt_bl) # 9970
id_ot_bl <- intersect(id_ot_bl, id_satlife_bl) # 9826
...
id_ot <- intersect(id_cesd, id_shlt) # 7834
id_ot <- intersect(id_ot, id_satlife) # 7594
...
filter_cov_iyes <- filter_nomental %>%
  ...
  ungroup()
length(unique(filter_cov_iyes$id)) # 5781
...
id_ot_2w <- intersect(id_cesd_2w, id_shlt_2w) # 7069
id_ot_2w <- intersect(id_ot_2w, id_satlife_2w) # 6946
...
id_ot <- intersect(id_cesd, id_shlt) # 6208
id_ot <- intersect(id_ot, id_satlife) # 6022
...
filter_cov <- filter_nomental %>%
  ...
  ungroup()
length(unique(filter_cov$id)) # 4559
...
length(unique(filter_pgs$id)) # 3051
...
save(
  final_iyes_elsa, final_ifreq_elsa, final_icum_elsa, final_iyes_pgs_elsa, # Main analyses
  ...
  married_iyes_elsa, nonmarried_iyes_elsa,
  phyinact_iyes_elsa, phyact_iyes_elsa, adl_iyes_elsa, noadl_iyes_elsa,
  dis_iyes_elsa, nodis_iyes_elsa, # Subgroup analyses
  mi_iyes_elsa, mi_ifreq_elsa, mi_icum_elsa, mi_iyes_pgs_elsa, # Multiple imputation analyses
