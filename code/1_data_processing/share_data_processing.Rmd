...
wide_harm_share <- harm_share %>%
  select(
    c(mergeid, hhid, pn, country, isocountry, ragender, raeducl)| # id, country, gender, education
      ...
      (starts_with("r") & (ends_with("alzdeme"))) | # Memory-related diseases, Alzheimer’s disease, dementia
...
long_share <- wide_harm_share %>% 
  select(!contains(names_reorder), contains(names_reorder)) %>%
    pivot_longer(
    col = !c(mergeid, hhid, pn, country, isocountry, ragender, raeducl),
    names_to = ".value",
...
id_ot_bl <- intersect(id_cesd_bl, id_shlt_bl) # 82709
id_ot_bl <- intersect(id_ot_bl, id_satlife_bl) # 82091
...
id_ot <- intersect(id_cesd, id_shlt) # 56042
id_ot <- intersect(id_ot, id_satlife) # 55925
...
filter_cov_iyes <- filter_nomental %>%
  ...
  ungroup()
length(unique(filter_cov_iyes$id)) # 44523
...
id_ot_2w <- intersect(id_cesd_2w, id_shlt_2w) # 47756
id_ot_2w <- intersect(id_ot_2w, id_satlife_2w) # 47511
...
id_ot <- intersect(id_cesd, id_shlt) # 27592
id_ot <- intersect(id_ot, id_satlife) # 27534
...
filter_cov <- filter_nomental %>%
  ...
  ungroup()
length(unique(filter_cov$id)) # 21137
...
length(unique(filter_cov$id)) # 21133
...
save(
  final_iyes_share, final_icum_share, # Main analyses
